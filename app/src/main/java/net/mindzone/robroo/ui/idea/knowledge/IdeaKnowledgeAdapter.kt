...
class IdeaKnowledgeAdapter(private val responseData: ResponseData,
                           val context: Context ):
        RecyclerView.Adapter<IdeaKnowledgeAdapter.MyViewHolder>() {
    var listFilter=responseData.items
...
//                itemListener(list.items[pos].contentid)
        ...
        fun onBind(string: Items) {
                ...
                Glide.with(context).load(string.imageth).centerCrop().error(R.drawable.default_image).into(imageView)
